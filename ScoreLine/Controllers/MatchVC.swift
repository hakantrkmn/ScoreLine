...
class MatchVC: UIViewController,UITableViewDelegate,UITableViewDataSource,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
    ...
    func getFixtureData(){
        let fixtureUrl = "https://\(apiHost)/fixtures?id=\(matchData["id"] as? Int ?? 0)"
    ...
    func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        let teamData = liveData["teams"] as? AnyObject
        let homeTeamFromLive = teamData?["home"] as? AnyObject
        let awayTeamFromLive = teamData?["away"] as? AnyObject
        if section == 0{
            let homeTeamData = homeTeam["team"] as? AnyObject
            return homeTeamData?["name"] as? String ?? homeTeamFromLive?["name"] as? String ?? "Home"
        }
        let awayTeamData = awayTeam["team"] as? AnyObject
        return awayTeamData?["name"] as? String ?? awayTeamFromLive?["name"] as? String ?? "Away"
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.lblTime.makeViewCurve(radius: cell.lblTime.frame.height/2)
